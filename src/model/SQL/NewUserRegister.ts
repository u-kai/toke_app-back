...
export class NewUserRegiser {
    ...
    SQLForInsertUserInfo = () => {
        return `INSERT INTO ${this.tableName} (user_id,user_name,image_path) VALUES((${this.SQLForGetCurrentId()}),'${
            this.userName
        }','')`
    }
    run = async () => {
        return await this.insertNewAndUpdateSeqUser.confirmIsNotExist().then((results: boolean) => {
            if (results === false) {
